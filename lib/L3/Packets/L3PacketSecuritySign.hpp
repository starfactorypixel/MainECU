...
{
	static constexpr L3C::format_t _format = L3C::FORMAT_SIGN;
	...
	public:
		...
		void Start()
		{
			esp_fill_random(_packet.payload_sign.iv, L3C::SECURITY_IV_LEN);
	...
	private:
		...
		void _Prepare()
		{
			...
			memmove(_packet.payload_sign.data + payload_len, _packet.payload_sign.tag, L3C::SECURITY_TAG_LEN);
		...
		void _Parse()
		{
			...
			memmove(_packet.payload_sign.tag, _packet.payload_sign.data + _packet.payload_len, L3C::SECURITY_TAG_LEN);
			// Возможно тут проблема, поскольку мы можем читать хвост tag уже за пределами data.
		...
		bool _Signing()
		{
			...
			uint16_t aadata_len = _packet_length - L3C::SECURITY_TAG_LEN;
