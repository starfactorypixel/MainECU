...
void ESP32SJA1000Class::onReceive(on_receive_t callback)
...
void ESP32SJA1000Class::handleInterrupt()
{
  ...
  if (ir & 0x01) {
    ...
    packet_t packet = 
    {
      (uint32_t)this->packetId(),
      this->packetExtended(),
      this->packetRtr(),
      (uint8_t)this->packetDlc(),
      (uint8_t)this->available()
    };
    
    uint8_t i = 0;
    int16_t data;
    #warning Or while(this->available() > 0) ???
    while(true)
    {
      data = this->read();
      if(data < 0) break;
